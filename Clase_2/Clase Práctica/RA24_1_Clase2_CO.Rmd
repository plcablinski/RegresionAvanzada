...
# Con ggplot graficar un diagrama de dispersión con la recta de regresión
ggplot(data = University, mapping = aes(x = stfees, y = nassets)) +
     geom_point(color = "firebrick", size = 2) +
     labs(title = "Diagrama de dispersión con recta de regresión", x = "stfees") +
...
boxCox(object = linMod, plotit = TRUE)
summary(powerTransform(linMod))
```
<br>
...
linMod3 <- lm(nassets**0.33 ~ stfees, data = University)
summary(linMod3)
...
plot(University$stfees,University$nassets**0.33,xlab="stfees",ylab="nassets^0.33",
     main="stfees vs nassets^0.33")

abline(linMod3,col="blue",lwd=2)
...
```{r, echo=TRUE}
linMod2b <- lm(nassets ~ log10(stfees), data = University)
boxcox(nassets ~ log10(stfees), lambda = -2:2, data = University)#lambda cerca de 0
boxCox(object = linMod2b, plotit = TRUE)
summary(powerTransform(linMod2b))
```
<br>
...
# Hacer un influenceIndexPlot y mostrar los 4 valores más altos
influenceIndexPlot(linMod) 
...
# Validacion de supuestos para cada modelo
shapiro.test(linMod$residuals)
bptest(linMod)
# Durbin Watson test
dwt(linMod)
...
shapiro.test(linMod_ww$residuals)
bptest(linMod_ww)
# Durbin Watson test
dwt(linMod_ww)

# Validacion de supuestos para cada modelo
shapiro.test(linMod_www$residuals)
bptest(linMod_www)
# Durbin Watson test
dwt(linMod_www)
