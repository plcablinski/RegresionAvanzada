...
# modelo completo
# Entrenar el modelo
modelo_completo <- lm(headcirc ~ ., data = data)
  
# Hacer predicciones
predictions_completas <- predict(modelo_completo, data)

  # Métricas de performance
print(data.frame( RMSE = RMSE(predictions_completas, data$headcirc),
                    R2 = R2(predictions_completas, data$headcirc),
                    MAE = MAE(predictions_completas, data$headcirc)))
  
# Evaluar el rendimiento del modelo
metrics_completo <- postResample(predictions_completas, data$headcirc)
...
# Dividir el dataset en training y testing
 ...
 # Evaluar el rendimiento del modelo
# Calcular métricas para model1 (usando la última iteración del bucle for)
metrics_model1 <- postResample(predictions1, testing_dataset$headcirc)
...
print(model2)
summary(model2)
# Calcular métricas para model2
metrics_model2 <- model2$results[1, c("RMSE", "Rsquared", "MAE")]
...
print(model3)
# Calcular métricas para model3
metrics_model3 <- model3$results[1, c("RMSE", "Rsquared", "MAE")]
...
print(model4)
# Calcular métricas para model4
metrics_model4 <- model4$results[1, c("RMSE", "Rsquared", "MAE")]
print(metrics_model4)
```
```{r, echo=TRUE}
set.seed(12345)
train_control5 <- trainControl(method = "repeatedcv", number = 10, repeats = 10)
model5 <- train(headcirc ~., data = data, method = "lm",trControl = train_control5)
print(model5)
# Calcular métricas para model5
metrics_model5 <- model5$results[1, c("RMSE", "Rsquared", "MAE")]
...
```{r, echo=TRUE}
options(width = 160)
comparison <- data.frame(modelo = c("modelo_completo",
                                    "validación_simple",
                                    "LOOCV",
                                    "K-fold",
                                    "Repeated K-fold 3",
...
# Agregar a comparison la Comparación de los modelos
comparison$RMSE <- c(metrics_completo[1], metrics_model1[1], metrics_model2[1], metrics_model3[1], metrics_model4[1], metrics_model5[1])
comparison$R2 <- c(metrics_completo[2], metrics_model1[2], metrics_model2[2], metrics_model3[2], metrics_model4[2], metrics_model5[2])
comparison$MAE <- c(metrics_completo[3], metrics_model1[3], metrics_model2[3], metrics_model3[3], metrics_model4[3], metrics_model5[3])

# Imprimir el data frame de comparación
print(comparison)
```