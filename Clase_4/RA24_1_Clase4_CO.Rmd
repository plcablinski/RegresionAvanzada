...
```{r, echo=TRUE}
xx <- model.matrix(headcirc ~ ., data = data)
head(xx) 
head(data)
...
Para obtener un ajuste mediante regresión Elastic Net se indica argumento alpha=0.5
...
```{r, echo=TRUE}
modelo_final_elastic_net <- glmnet(x=x, y=y, alpha=0.5,lambda=cv_error_elastic_net$lambda.1se) 
...
abline(h = 7)
plot(cv_error_elastic_net, ylab = "Mean Square Elastic Net") 
abline(h = 7)
par(mfrow = c(1, 1))
```

Comparación de los coeficientes obtenidos con Ridge y Lasso.

```{r, echo=TRUE}
min_mse_ridge <- min(cv_error_ridge$cvm)
min_mse_ridge
min_mse_lasso <- min(cv_error_lasso$cvm)
min_mse_lasso
min_elastic_net <- min(cv_error_elastic_net$cvm)
min_elastic_net

# Indicar el  el mínimo test-error con que método se consigue y cual es el valor de MSE
if (min_mse_ridge < min_mse_lasso & min_mse_ridge < min_elastic_net) {
  min_mse <- min_mse_ridge 
  metodo <- "Ridge"
} else if (min_mse_lasso < min_mse_ridge & min_mse_lasso < min_elastic_net) {
  min_mse <- min_mse_lasso
  metodo <- "Lasso"
} else {
  min_mse <- min_elastic_net
  metodo <- "Elastic Net"
}

print(paste("El método con menor MSE es", metodo, "con un MSE de", min_mse))
...
test_MSE_OLS
caret_MSE_OLS <- RMSE(pred = predict(modelo_OLS, data_2), obs = data_2$headcirc)^2 
caret_MSE_OLS 
```
